...
st.markdown("**My Main Idea:** Humans are too slow to stop hackers now. We need to spend money to use smart AI to defend our cloud. The data shows that when we are slow, we lose massive gigabytes of data, and hackers attack at specific times to break our old tools.")
...
with chart_col1:
    ...
    st.plotly_chart(px.scatter(plot_df, x="response_time_min", y="data_compromised_GB", color="outcome", size="attack_severity", opacity=0.5, color_discrete_map={"Success": "#d73027", "Failed": "#74add1", "success": "#d73027", "failed": "#74add1"}, hover_data=["attack_type", "target_system"]).update_layout(title=dict(text="Cost of Slow Response: Time vs. Data Lost", x=0.5, xanchor='center'), margin=dict(l=0, r=0, b=0, t=40)), use_container_width=True)
...
with chart_col2:
    ...
    st.plotly_chart(px.density_heatmap(plot_df, x="Hour", y="Day", z="Is_Attack", histfunc="sum", color_continuous_scale="Viridis").update_layout(title=dict(text="Peak Vulnerability: Attacks by Day and Hour", x=0.5, xanchor='center'), margin=dict(t=40)), use_container_width=True)
...
with chart_col3:
    ...
    st.plotly_chart(px.treemap(plot_df.dropna(subset=['target_system', 'attack_type', 'security_tools_used']), path=[px.Constant("Network"), 'target_system', 'attack_type', 'security_tools_used'], values='Count', color='Is_Attack', color_continuous_scale='Reds').update_layout(title=dict(text="Weakest Links: Failing Security Tools", x=0.5, xanchor='center'), margin=dict(t=40)), use_container_width=True)
